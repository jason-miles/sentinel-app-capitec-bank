-- Capitec Sentinel — bank-grade governance: PII column masking (NEXT_STEPS #5).
--
...
-- ── Mask functions ────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION capitec_fraud_aml_gold.mask_email(v STRING)
RETURN CASE WHEN is_account_group_member('aml_pii_reviewers') THEN v
            WHEN v IS NULL THEN NULL
            ELSE regexp_replace(v, '^[^@]+', '****') END;

CREATE OR REPLACE FUNCTION capitec_fraud_aml_gold.mask_phone(v STRING)
RETURN CASE WHEN is_account_group_member('aml_pii_reviewers') THEN v
            WHEN v IS NULL THEN NULL
            ELSE concat('*******', right(v, 3)) END;

CREATE OR REPLACE FUNCTION capitec_fraud_aml_gold.mask_dob(v DATE)
RETURN CASE WHEN is_account_group_member('aml_pii_reviewers') THEN v
            WHEN v IS NULL THEN NULL
            ELSE date_trunc('YEAR', v) END;

CREATE OR REPLACE FUNCTION capitec_fraud_aml_gold.mask_addr(v STRING)
RETURN CASE WHEN is_account_group_member('aml_pii_reviewers') THEN v
            WHEN v IS NULL THEN NULL
            ELSE '*** REDACTED ***' END;

-- ── Apply masks ─────────────────────────────────────────────────────────────
ALTER TABLE capitec_fraud_aml_bronze.customers
  ALTER COLUMN email   SET MASK capitec_fraud_aml_gold.mask_email;
ALTER TABLE capitec_fraud_aml_bronze.customers
  ALTER COLUMN phone   SET MASK capitec_fraud_aml_gold.mask_phone;
ALTER TABLE capitec_fraud_aml_bronze.customers
  ALTER COLUMN dob     SET MASK capitec_fraud_aml_gold.mask_dob;
ALTER TABLE capitec_fraud_aml_bronze.customers
  ALTER COLUMN address SET MASK capitec_fraud_aml_gold.mask_addr;

ALTER TABLE capitec_fraud_aml_bronze.third_parties
  ALTER COLUMN address SET MASK capitec_fraud_aml_gold.mask_addr;
