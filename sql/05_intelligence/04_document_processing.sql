...
CREATE OR REPLACE TABLE elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_gold.document_extractions AS
WITH docs AS (
  SELECT
    _metadata.file_path AS file_path,
    CAST(content AS STRING) AS doc_text
  FROM READ_FILES(
    '/Volumes/elexon_app_for_settlement_acc_catalog/capitec_fraud_aml_bronze/documents/*.txt',
    format => 'text',
    wholeText => true
  )
)
