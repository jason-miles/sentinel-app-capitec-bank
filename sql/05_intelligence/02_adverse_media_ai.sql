...
CREATE OR REPLACE TABLE elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_gold.adverse_media_analysis AS
WITH matches AS (
  SELECT e.entity_id, e.full_name, e.party_type,
         m.article_id, m.headline, m.body, m.source, m.published_at
  FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.entities e
  JOIN elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.adverse_media m
