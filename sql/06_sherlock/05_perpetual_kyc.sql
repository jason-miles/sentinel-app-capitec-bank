...
CREATE OR REPLACE VIEW capitec_fraud_aml_gold.pkyc_customer_risk AS
WITH cust AS (
  ...
  FROM capitec_fraud_aml_silver.customers c
  LEFT JOIN capitec_fraud_aml_silver.entity_map em
...
turnover_sig AS (
  ...
  FROM capitec_fraud_aml_silver.transactions t
  JOIN capitec_fraud_aml_silver.accounts a USING (account_id)
...
alert_sig AS (
  ...
  FROM capitec_fraud_aml_gold.fraud_alerts fa
  JOIN capitec_fraud_aml_silver.entity_map em
...
sanc_sig AS (
  ...
  FROM capitec_fraud_aml_gold.sanctions_screening_hits
  WHERE party_type = 'customer'
  GROUP BY source_id
),
media_sig AS (
  SELECT em.source_id AS customer_id, count(*) AS media_hits
  FROM capitec_fraud_aml_gold.adverse_media_analysis am
  JOIN capitec_fraud_aml_silver.entity_map em
...
acct_sig AS (
  ...
  FROM capitec_fraud_aml_silver.accounts GROUP BY customer_id
),
scored AS (
  SELECT
