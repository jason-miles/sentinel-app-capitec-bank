...
-- ── Watchlist ────────────────────────────────────────────────────────────
CREATE OR REPLACE TABLE capitec_fraud_aml_gold.sanctions_watchlist AS
SELECT * FROM VALUES
...
CREATE OR REPLACE TABLE capitec_fraud_aml_gold.sanctions_screening_hits AS
WITH entities AS (
...
scored AS (
  ...
  FROM entities e
  CROSS JOIN capitec_fraud_aml_gold.sanctions_watchlist w
),
matched AS (
  SELECT *,
