...
FROM capitec_fraud_aml_gold.sanctions_screening_hits h
LEFT JOIN capitec_fraud_aml_silver.customers cust ON cust.customer_id = h.source_id;
...
SELECT
  ...
  0.0                                                                      AS investigation_hours,
  current_timestamp()                                                      AS opened_at
FROM capitec_fraud_aml_gold.peer_anomaly a;
