-- Capitec Sentinel — app write-back tables (create-if-not-exists).
--
...
--
-- Schema: elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_gold

USE CATALOG elexon_app_for_settlement_acc_catalog;

CREATE TABLE IF NOT EXISTS capitec_fraud_aml_gold.sherlock_case_notes (
  note_id    STRING,
  case_id    STRING,
  author     STRING,
  note       STRING,
...
CREATE TABLE IF NOT EXISTS capitec_fraud_aml_gold.sherlock_case_actions (
  action_id  STRING,
  case_id    STRING,
...
CREATE TABLE IF NOT EXISTS capitec_fraud_aml_gold.sherlock_sar_filings (
  sar_id        STRING,
  case_id       STRING,
...
VALUES
  ('NOTE-9001A','CASE-90001','Thandeka Nkosi','Alert opened: three cash deposits just under R25k over ~40h on a recently-opened Global One account. Checking for linked accounts.','analyst', current_timestamp() - INTERVAL 3 HOURS),
...
INSERT INTO capitec_fraud_aml_gold.sherlock_case_actions
