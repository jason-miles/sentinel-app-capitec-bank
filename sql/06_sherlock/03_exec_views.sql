...
CREATE OR REPLACE VIEW capitec_fraud_aml_gold.sherlock_exec_kpis AS
SELECT
...
FROM capitec_fraud_aml_gold.sherlock_cases;

-- Alerts by scenario (horizontal bar).
CREATE OR REPLACE VIEW capitec_fraud_aml_gold.sherlock_by_scenario AS
SELECT scenario, count(*) AS alerts FROM capitec_fraud_aml_gold.sherlock_cases
GROUP BY scenario ORDER BY alerts DESC;

-- Alerts by priority x status (heatmap).
CREATE OR REPLACE VIEW capitec_fraud_aml_gold.sherlock_priority_status AS
SELECT priority, status, count(*) AS alerts FROM capitec_fraud_aml_gold.sherlock_cases
GROUP BY priority, status;

-- Case resolution flow (Sankey): scenario -> team -> status.
CREATE OR REPLACE VIEW capitec_fraud_aml_gold.sherlock_resolution_flow AS
SELECT scenario AS source, team_name AS target, count(*) AS value
FROM capitec_fraud_aml_gold.sherlock_cases GROUP BY scenario, team_name
UNION ALL
SELECT team_name AS source, status AS target, count(*) AS value
FROM capitec_fraud_aml_gold.sherlock_cases GROUP BY team_name, status;

-- Team performance (Team Performance tab).
CREATE OR REPLACE VIEW capitec_fraud_aml_gold.sherlock_team_performance AS
SELECT team_name,
       count(*) AS cases,
...
FROM capitec_fraud_aml_gold.sherlock_cases GROUP BY team_name;

-- Daily new alerts (area chart) — cases opened per day.
CREATE OR REPLACE VIEW capitec_fraud_aml_gold.sherlock_daily_new AS
SELECT date(opened_at) AS d, count(*) AS alerts
FROM capitec_fraud_aml_gold.sherlock_cases GROUP BY date(opened_at);

-- Outstanding alerts by due date (bar chart) — open cases bucketed by due date.
CREATE OR REPLACE VIEW capitec_fraud_aml_gold.sherlock_outstanding AS
SELECT due_date, count(*) AS alerts
FROM capitec_fraud_aml_gold.sherlock_cases WHERE status <> 'closed'
GROUP BY due_date;
