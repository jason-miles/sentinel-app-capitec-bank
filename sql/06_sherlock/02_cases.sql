...
CREATE OR REPLACE TABLE capitec_fraud_aml_gold.sherlock_cases AS
WITH base AS (
  SELECT
    id,
...
LEFT JOIN capitec_fraud_aml_silver.customers c ON c.customer_id = b.customer_id;

-- ── HERO CASES for the demo "wow" narratives ─────────────────────────────
-- Anchored to the planted subjects so a demo driver can open them by name.
-- These are the cases the walkthrough (docs/capitec_demo_walkthrough.md) drives:
