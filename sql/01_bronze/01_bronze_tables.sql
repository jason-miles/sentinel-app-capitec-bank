-- Capitec Fraud & AML — Bronze layer DDL
...
--
-- Schema: elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_bronze

USE CATALOG elexon_app_for_settlement_acc_catalog;

-- ── Core entity: CUSTOMER ────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS capitec_fraud_aml_bronze.customers (
  customer_id        STRING,
  full_name          STRING,
  dob                DATE,
  ...
  city               STRING,
  country            STRING,
  segment            STRING,      -- Global One product tier
...
-- ── Core entity: ACCOUNT ─────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS capitec_fraud_aml_bronze.accounts (
  account_id             STRING,
  customer_id            STRING,
...
CREATE TABLE IF NOT EXISTS capitec_fraud_aml_bronze.transactions (
...
CREATE TABLE IF NOT EXISTS capitec_fraud_aml_bronze.card_transactions (
  card_txn_id  STRING,
  card_id      STRING,
  account_id   STRING,
  amount       DOUBLE,
...
CREATE TABLE IF NOT EXISTS capitec_fraud_aml_bronze.third_parties (
...
CREATE TABLE IF NOT EXISTS capitec_fraud_aml_bronze.risk_ratings (
  rating_id    STRING,
...
CREATE TABLE IF NOT EXISTS capitec_fraud_aml_bronze.beneficial_ownership (
  ubo_id         STRING,
...
CREATE TABLE IF NOT EXISTS capitec_fraud_aml_bronze.auth_events (
  event_id         STRING,
  account_id       STRING,
  event_ts         TIMESTAMP,
  new_device       BOOLEAN,
...
CREATE TABLE IF NOT EXISTS capitec_fraud_aml_bronze.adverse_media (
  article_id   STRING,
  headline     STRING,
  body         STRING,
  published_at DATE,
...
CREATE TABLE IF NOT EXISTS capitec_fraud_aml_bronze.aml_knowledge (
  doc_id       STRING,
...
CREATE TABLE IF NOT EXISTS capitec_fraud_aml_bronze.sar_narratives (
  sar_id       STRING,
  filed_at     DATE,
  typology     STRING,
