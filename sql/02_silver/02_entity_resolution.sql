-- Capitec Fraud & AML — Silver: ENTITY RESOLUTION (PRD §5, "the key")
...
CREATE OR REPLACE TABLE capitec_fraud_aml_silver.parties_raw AS
SELECT
  customer_id       AS source_id,
  'customer'        AS party_type,
  full_name, national_id, tax_number, city, country, dob
FROM capitec_fraud_aml_silver.customers
UNION ALL
SELECT
...
FROM capitec_fraud_aml_silver.third_parties;
...
CREATE OR REPLACE TABLE capitec_fraud_aml_silver.entities AS
WITH keyed AS (
  SELECT *,
  ...
  FROM capitec_fraud_aml_silver.parties_raw
),
clustered AS (
  SELECT *,
...
CREATE OR REPLACE TABLE capitec_fraud_aml_silver.entity_map AS
SELECT source_id, party_type, entity_id, cluster_size
FROM capitec_fraud_aml_silver.entities;
