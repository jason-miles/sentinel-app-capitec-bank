...
INSERT OVERWRITE capitec_fraud_aml_bronze.risk_ratings
...
INSERT OVERWRITE capitec_fraud_aml_bronze.beneficial_ownership
WITH tp AS (
...
INSERT OVERWRITE capitec_fraud_aml_bronze.auth_events
WITH acct AS (
  SELECT account_id, cast(regexp_replace(account_id,'^ACC0*','') AS BIGINT) AS acct_num
  FROM capitec_fraud_aml_bronze.accounts WHERE status='active'
),
ev AS (
...
INSERT OVERWRITE capitec_fraud_aml_bronze.adverse_media
SELECT * FROM VALUES
