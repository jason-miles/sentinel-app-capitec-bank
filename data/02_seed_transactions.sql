-- Capitec Fraud & AML — Synthetic seeder (2/4): bulk ledger + card transactions
-- Target ~2-3M ledger transactions over the trailing 12 months (PRD §10).
-- Capitec context: retail channels skew to the banking app, card, ATM cash and
-- Cash Send; descriptions reflect everyday retail spend and salary/SASSA credits.
...
SELECT
  ...
  element_at(array('Card purchase','Salary credit','SASSA grant','Cash Send','Prepaid airtime','DebiCheck debit order','ATM withdrawal','EFT payment'), cast(pmod(txn_seq,8)+1 AS INT)) AS description,
  concat('DEV', lpad(cast(pmod(acct_num * 40009, 90000000) AS BIGINT), 8, '0')) AS device_id,  -- customer's own device
