-- Capitec Bank — Fraud & AML SAMPLE DATA · Synthetic seeder (2/4): bulk ledger + card
-- transactions.  *** CAPITEC BANK DEMO DATA — 100% SYNTHETIC ***
-- Target ~2-3M ledger transactions over the trailing 12 months (PRD §10).
-- Capitec context: retail channels skew to the Global One banking app, card, ATM cash
...
INSERT OVERWRITE capitec_fraud_aml_bronze.transactions
WITH acct AS (
...
txn AS (
  ...
  FROM acct a
  -- ~30-60 txns/account (~500k rows) — enough for realistic distributions + all detection
  -- rules, without the storage/compute cost of the original 2.4M. Planted scenarios (file 04)
  -- are unaffected. To restore the heavier volume, bump these back to 150 + pmod(...,120).
...
SELECT
  ...
  element_at(array('Card purchase','Salary credit','SASSA grant','Capitec Cash Send','Global One transfer','Prepaid airtime & data','DebiCheck debit order','ATM cash withdrawal','Immediate payment (RTC)'), cast(pmod(txn_seq,9)+1 AS INT)) AS description,
  concat('GO-APP-', lpad(cast(pmod(acct_num * 40009, 90000000) AS BIGINT), 8, '0')) AS device_id,  -- client's own Global One app device
...
INSERT OVERWRITE capitec_fraud_aml_bronze.card_transactions
WITH cards AS (
  SELECT account_id, customer_id,
         cast(regexp_replace(account_id, '^ACC0*', '') AS BIGINT) AS acct_num
  FROM capitec_fraud_aml_bronze.accounts
