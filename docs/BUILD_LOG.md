# Capitec Sentinel — Build Log

**Product:** Capitec Sentinel — CDP & Financial Crime Intelligence Platform
...
**Live app:** https://capitec-sentinel-7474654808133980.aws.databricksapps.com
...
An end-to-end, SQL-first Anti-Money-Laundering + CDP platform for Capitec Wealth &
...
`capitec_fraud_aml_bronze` / `_silver` / `_gold`. (Logical target was a dedicated
`capitec_fraud_aml` catalog; only the schema prefix differs.)
...
## Rebuild — "Capitec Sentinel" (advanced, per SherlockAML reference)

Renamed from the interim "SherlockAML" reference build to **Capitec Sentinel**.
...
- **Design = capitec.com**: Libre Caslon Text serif + Inter, slate-navy `#30384a`
  + gold `#c9a24b`, near-square corners, spectacular inline-SVG "Capitec | Sentinel"
  hero logo.
...
- Root `databricks.yml` (bundle `capitec_sentinel`) unifying app+pipeline+jobs (migration pending — #6). `.github/workflows/` (ci + deploy; parked on `ci-workflows` branch — see `ci_cd.md`). Tests: **32 pytest** (`app/backend/tests/`) + **6 Playwright e2e** (`frontend/e2e/`). Lakeflow **DQ expectations on every dataset**.
...
| Schemas | `capitec_fraud_aml_{bronze,silver,gold}` |
| App | `capitec-sentinel` |
| App service principal | `982e92ba-63ff-4de6-95ff-2bea54a734bd` |
| Genie space | `<capitec-genie-space-id>` ("Fraud & AML Analyst", 8 tables) |
...
| AI/BI dashboard | `<capitec-dashboard-id>` ("Capitec Sentinel — Executive Overview") |
| Landing volume | `...capitec_fraud_aml_bronze.landing/{transactions,card_transactions}/` |
...
8. **App**: build the frontend (`cd app/backend/frontend && npm ci && npm run build`),
   copy `dist` → `webroot`, `databricks apps create capitec-sentinel`, sync
