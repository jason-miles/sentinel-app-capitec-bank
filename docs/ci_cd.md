# CI/CD — Capitec Sentinel (roadmap #5)
...
> `capitec_sentinel/…`, distinct from the live nested `fraud_aml_pipeline` bundle —
...
GRANT USE SCHEMA, SELECT, MODIFY ON SCHEMA elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_bronze TO `<CICD_SP_APP_ID>`;
GRANT USE SCHEMA, SELECT, MODIFY ON SCHEMA elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver TO `<CICD_SP_APP_ID>`;
GRANT USE SCHEMA, SELECT, MODIFY, CREATE TABLE, CREATE FUNCTION ON SCHEMA elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_gold TO `<CICD_SP_APP_ID>`;
```
