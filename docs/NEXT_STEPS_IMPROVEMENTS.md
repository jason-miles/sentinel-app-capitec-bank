# Capitec Sentinel — Next Steps & Improvement Ideas
...
- React + FastAPI Databricks App (Capitec Sentinel): Executive Overview, Alert
...
- **Consolidate to a bundle-deployed app** (databricks.yml `app` resource) so the app,
  ...
  - ✅ **DONE (2026-07-20).** Root `databricks.yml` (bundle `capitec_sentinel`)
    unifies the app (`resources/capitec_app.app.yml` — adopts the existing
    `capitec-fraud-aml` app by name) + pipeline + jobs via includes, with dev/prod
...
- **Accessibility & i18n**, dark mode, and an embedded AI/BI dashboard on the Reports
  ...
  - ✅ **Embedded AI/BI dashboard DONE (2026-07-20).** Published Lakeview dashboard
    "Capitec Sentinel — Executive Overview" (KPI counters + daily-alerts line +
