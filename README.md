# Capitec Bank — Fraud & AML Detection App on Databricks

A demo/PoC that answers, end-to-end: **"What kinds of alerts would indicate financial crime at Capitec, and what do we do about them?"** — framed for a South African mass-market retail bank (Global One accounts, ZAR, FICA/FIC regulatory regime).

Built SQL-first (Lakeflow Declarative Pipelines) so a SQL-skilled team can own it. Python is confined to the app shell and a few AI/graph helpers. Competitive context for the Capitec account: a best-of-breed data + AI layer on their existing **AWS** estate (coexistence, not rip-and-replace) — see `docs/`.
...
- **A — Hidden mule network:** a "boring" sub-threshold cash-structuring alert on one Global One account expands, via entity resolution (shared device / IP / address, opened within a 3-week window), into a **7-account money-mule network** forwarding ~90% within 48h to one aggregator that remits cross-border SWIFT — with sibling accounts previously closed as false positives.
...
The PRD calls for a dedicated `capitec_fraud_aml` catalog with `bronze` / `silver` / `gold` schemas. The demo workspace user (`jason.miles@databricks.com`) has **ALL PRIVILEGES on `elexon_app_for_settlement_acc_catalog`** but **no `CREATE CATALOG` on the metastore** (`metastore_aws_eu_central_1`). Following the established Valterra precedent in this workspace, the medallion is co-located as prefixed schemas inside the existing catalog:

| Logical (PRD)          | Physical (this workspace)                                           |
|------------------------|---------------------------------------------------------------------|
| `capitec_fraud_aml.bronze` | `elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_bronze` |
| `capitec_fraud_aml.silver` | `elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver` |
| `capitec_fraud_aml.gold`   | `elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_gold`   |

All SQL references the physical names. If a real `capitec_fraud_aml` catalog becomes available, a find/replace of the schema prefix is the only change needed.
