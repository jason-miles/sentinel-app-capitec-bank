# Declarative Automation Bundles Project — Capitec Fraud & AML
...
Lakeflow Declarative Pipeline (SQL, serverless) for the Capitec Wealth & Banking
...
Default catalog/schema: elexon_app_for_settlement_acc_catalog / capitec_fraud_aml_gold.
Silver datasets use fully-qualified names to publish into capitec_fraud_aml_silver.

## Streaming lanes (near-real-time hot-path)

Two Auto Loader streaming lanes land JSON in the UC Volume
`capitec_fraud_aml_bronze.landing/<lane>/` and ingest incrementally via `read_files`
...
Most silver and gold datasets publish to *different schemas* (capitec_fraud_aml_silver
