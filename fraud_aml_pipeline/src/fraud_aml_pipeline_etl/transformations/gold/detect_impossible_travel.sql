...
ordered AS (
  ...
  FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.card_transactions
),
speeds AS (
  SELECT *,
...
FROM speeds s
CROSS JOIN cfg
LEFT JOIN elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.accounts a ON a.account_id = s.account_id
