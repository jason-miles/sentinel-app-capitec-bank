...
WITH cfg AS (SELECT * FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_gold.alert_config),
anomalies AS (
  SELECT ae.account_id, ae.event_ts, ae.new_device, ae.new_geo, ae.credential_change, ae.geo_city
  FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.auth_events ae
...
drains_raw AS (
  ...
  FROM anomalies an
  JOIN elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.transactions t
...
FROM drains d
LEFT JOIN elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.accounts a ON a.account_id = d.account_id
