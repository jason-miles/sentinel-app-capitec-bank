...
CREATE OR REFRESH MATERIALIZED VIEW elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_gold.customer_360 AS
WITH acct_rollup AS (
  SELECT customer_id, count(*) AS num_accounts, sum(balance) AS total_balance
  FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.accounts
...
latest_risk AS (
  SELECT entity_id, risk_rating, rated_at FROM (
    ...
    FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.risk_ratings
...
alert_counts AS (
  ...
  FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_gold.fraud_alerts fa
  JOIN elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.entity_map em
...
turnover AS (
  ...
  FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.transactions t
  JOIN elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.accounts a USING (account_id)
...
FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.customers c
LEFT JOIN elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.entity_map em
