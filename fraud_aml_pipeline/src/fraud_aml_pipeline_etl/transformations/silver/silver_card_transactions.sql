...
CREATE OR REFRESH MATERIALIZED VIEW
  elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.card_transactions (
...
SELECT * EXCEPT (rn) FROM (
  ...
  FROM (
    ...
    FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_bronze.card_transactions
    UNION ALL
    SELECT card_txn_id, card_id, account_id, amount, currency, merchant, merchant_category,
           channel, lat, lon, city, country, txn_ts, _ingested_at
    FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_bronze.card_transactions_stream
  )
) WHERE rn = 1;
