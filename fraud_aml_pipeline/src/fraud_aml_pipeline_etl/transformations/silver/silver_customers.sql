...
CREATE OR REFRESH MATERIALIZED VIEW elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.customers (
...
SELECT * EXCEPT (rn) FROM (
  ...
  FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_bronze.customers
) WHERE rn = 1;
