...
CREATE OR REFRESH MATERIALIZED VIEW elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.accounts (
...
SELECT * EXCEPT (rn) FROM (
  ...
  FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_bronze.accounts
) WHERE rn = 1;
