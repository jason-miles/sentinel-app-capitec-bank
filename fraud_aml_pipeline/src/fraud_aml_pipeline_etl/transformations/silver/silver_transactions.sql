...
CREATE OR REFRESH MATERIALIZED VIEW
  elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.transactions (
...
SELECT * EXCEPT (rn) FROM (
  ...
  FROM (
    ...
    FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_bronze.transactions
    UNION ALL
    SELECT transaction_id, account_id, from_acct, to_acct, direction, amount,
           currency, counterparty_id, channel, merchant_category, txn_ts, description,
           device_id, ip_address, is_cross_border, source_system, _ingested_at
    FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_bronze.transactions_stream
  )
) WHERE rn = 1;
