...
CREATE OR REFRESH MATERIALIZED VIEW elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.entities (
...
WITH parties_raw AS (
  ...
  FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.customers
  UNION ALL
  ...
  FROM elexon_app_for_settlement_acc_catalog.capitec_fraud_aml_silver.third_parties
),
keyed AS (
  SELECT *,
